...
This project integrates a collection of New Mexico political geography and election data into a unified dataset.


## Dataset Components

### Precinct Boundaries with District Crosswalk
Voting Tabulation Districts (VTDs) from the New Mexico Secretary of State's 2020 redistricting cycle. Spatial geometries defining precinct boundaries with county and VTD identifiers, including congressional district, state house district, and state senate district assignments as properties.

**File:** `data/boundaries/nm_vtd_with_districts_2021.geojson`  
**Format:** GeoJSON with precinct geometries and district assignments

### Precinct-Level Election Results
Precinct-by-precinct vote totals broken down by party (Democratic, Republican, and other) for all Congressional races, State legislative races, and Statewide offices/elections  (Governor, U.S. Senate, presidential electors, and other statewide positions) from 2000-2024. 

Derived from election information made available at https://electionstats.sos.nm.gov/.

**File:** `data/elections/nm_precinct_level_results_2000-24.csv`  
**Format:** CSV with vote totals by precinct, election year, and party


### Statewide Election Results
Final statewide vote tallies, winners, and vote percentages for all races from 2000-2024. Includes presidential, gubernatorial, U.S. Senate, U.S. House, state legislative, and other statewide offices. 

Derived from election information made available at https://electionstats.sos.nm.gov/.

**File:** `data/elections/nm_statewide_results_2000-24.csv`  
**Format:** CSV with race outcomes and vote shares by election year
...
sf::st_write(nm_vtd_with_districts, "data/boundaries/nm_vtd_with_districts_2021.geojson", delete_dsn = TRUE)
write.csv(precinct_level, "data/elections/nm_precinct_results_2000-24.csv", row.names = FALSE)
write.csv(final_results, "data/elections/nm_election_results_2000-24.csv", row.names = FALSE)
```
...
# Load precinct boundaries with district crosswalk
nm_vtd_with_districts <- sf::st_read("data/boundaries/nm_vtd_with_districts_2021.geojson")

# Load precinct-level State House results
statehouse_results <- read.csv("data/elections/nm_precinct_results_statehouse_2000-24.csv")

# Load precinct-level Congressional results
congress_results <- read.csv("data/elections/nm_precinct_results_congress_2000-24.csv")

# Load statewide results
statewide_results <- read.csv("data/elections/nm_statewide_results_2000-24.csv")
```
